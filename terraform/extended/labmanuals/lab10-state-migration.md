...
> **Goal:** Migrate existing local state to an approved S3 backend.
> **Time:** ~40 min · **Files:** `labs/lab10-state-migration/`
...
- Run every command from `terraform/extended/labs/lab10-state-migration`.
...
---
*Deep dive: [docs/state/README.md](docs/state/README.md). Next: [Lab 11](lab11-remote-state-consumer.md)*

## Operational checklist

### Control 1 — Migration runbook

Take a local state backup and store it in an approved encrypted location before migration.
...
### Control 2 — Migration runbook

Confirm the destination bucket, key, region, encryption, and access policy independently.
...
### Control 3 — Migration runbook

Run migration from a clean working directory with the intended workspace selected.
...
### Control 4 — Migration runbook

Do not proceed if Terraform proposes a different source or destination than expected.
...
### Control 5 — Migration runbook

After migration, verify both the backend and resource address list before planning changes.
