...
> **Goal:** Generate repeatable nested security-group rules from a typed input map.
> **Time:** ~20 min · **Files:** `labs/lab14-dynamic-blocks/`
...
- Run every command from `terraform/extended/labs/lab14-dynamic-blocks`.
...
Use `terraform console` to test transformations independently. Stable map keys make `for_each` resource addresses predictable. Dynamic blocks are appropriate only when the provider schema genuinely needs repeated nested blocks; a normal `for_each` resource is clearer whenever the platform supports it.
...
---
*Deep dive: [docs/functions/README.md](docs/functions/README.md). Next: [Lab 15](lab15-capstone-projects.md)*
