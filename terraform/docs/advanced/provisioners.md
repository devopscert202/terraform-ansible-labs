...
Deep dive for lab14 and lab15. Covers what provisioners do, why HashiCorp calls them a last resort,
the two kinds you will meet, and what to use instead in almost every case.
...
your own reachable host: it isolates the SSH mechanics from the networking, which lab21 handles
separately.
...
|---|---|
| Install packages, write files, start services on a new VM | **`user_data`** — cloud-init, runs on the instance at first boot with no network path from Terraform. This is what lab21 does |
...
`user_data` deserves the emphasis. Compare lab15's `remote-exec` with what lab21 does:
...
- The declarative alternative in a full build: [`projects.md`](projects.md) and lab21's `user_data`
- Remote state, which lab16 onwards depends on: [`state.md`](state.md)
