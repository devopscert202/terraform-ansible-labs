...
Deep dive for lab05. Covers the three checks that belong in front of every `apply`, their exact
exit codes, and precisely how far a `sensitive` output is hidden — which is less far than most
people assume.
...
- Whether `t3.nano` is offered in `us-east-1`
...
[`../intermediate/07-state.md`](../intermediate/07-state.md).
...
- Variables, and how `sensitive` behaves on inputs as well as outputs:
  [`../intermediate/06-variables.md`](../intermediate/06-variables.md)
