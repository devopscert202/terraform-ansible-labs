...
| **3** | ~4 h | AWS concepts, credentials, providers, first EC2 in its own VPC, the core workflow, quality gates | [terraform](../terraform/labmanuals/README.md) `lab00`–`lab05` |
| **4** | ~4 h | Variables and secrets, state, modules, the capstone build | terraform `lab06`–`lab10` |
...
For the Terraform days, also read the [AWS primer](../terraform/html/aws-primer.html) and keep
[concepts.html](../terraform/html/concepts.html) open beside the labs.
...
| An S3 bucket as a managed resource | terraform `lab23` | After `lab22`, once buckets are familiar as state stores |
| `count` versus `for_each` on real buckets | terraform `lab24` | Last, closing the track |
